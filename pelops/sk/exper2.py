...
IN_DIR = '/Users/schuylerx/dev/data'
ITEMS_PER_CAMERA = 10
Y_RANDOM = 1024
CAMERAS = 2
DROPPED = 0
...
for i in range(1, 8):
    ...
    DATA_DIR = '%s/transforms/transform_%d' % (IN_DIR, i)
    HDF_FILE = '%s/transforms/transform_%d.resnet.hdf5' % (IN_DIR, i)
    print(DATA_DIR)
    ...
    results.append({'gdata': gdata, 'CMC_CNT': CMC_CNT, 'EXP_CNT': EXPERIMENTS, 'plot_name': plot_name}

with open(IN_DIR + '/experiment_results.pkl', 'wb') as ohdl:
    pickle.dump(results, ohdl)
